...
- `fetch_player_stats(player, season)` must become two-step: (1) if not already known, read the player's own profile raw file (already on disk from `fetch_player_profile`) to find the `entryId` matching `tournamentId == season.source_competition_id` and `seasonName` matching the target season label; (2) fetch `https://www.fotmob.com/api/data/playerStats?playerId={id}&seasonId={entryId}` and store _that_ as the `player_stats` raw artifact instead of re-fetching the profile page.
...
- Every `Club` must resolve to a `Country` row (ISO3 `country_code` from the club raw file) before insert — `Country` table needs an ISO3-keyed lookup/seed step, not just the country _name_ strings already used for `Competition.country`.
...
- [x] **1. Fix `scraper/adapters/fotmob.py::fetch_player_stats`** (the entryId-based endpoint). Verified live against Saka (961995): correct 2025-2026 season stats returned (7 goals, 5 assists, 2225 minutes, 31 appearances).
- [ ] **2. Re-run `player_stats`-stage-only crawl for all 572 players** against the fix (profile/`player` data doesn't need re-fetching) — `python -m scraper.cli crawl --source fotmob --competition epl --stage fetch-stats --force`. Only Saka has been re-fetched so far (for the single-player validation below).
- [x] **3. Add `PlayerMarketValue`/`PlayerAttributeProfile` to `db/models.py`**, generate + apply Alembic migration. Also widened `Player.primary_position` (`String(16)` → `String(64)`, was truncating labels like "Attacking Midfielder") and added `Player.height_cm` — found necessary during implementation, not anticipated in the original plan.
- [x] **4. Build `etl/models_raw/fotmob.py` + `etl/parsers/fotmob_parser.py`.** Not yet unit-tested against committed fixtures (still validated via a live ad-hoc script only) — fixture tests are still open work. One real gap found and fixed mid-implementation: `minutes_played`/`appearances`/`player_started_matches`/`rating` live only in the stats-endpoint's `topStatCard` section, not `statsSection` (which was all the parser originally read) — parser now merges both, keyed by `localizedTitleId`, `statsSection` winning on overlap.
- [x] **5. Build normalizers + loaders.** `etl/normalizers/{club,player,player_stats}.py`, `etl/load/loaders.py` built and validated. One inconsistency found and fixed: `Player.nationality` was storing FotMob's raw federation code (`"ENG"`) instead of a resolved friendly name (`"England"`) — now uses the same `FOOTBALL_COUNTRY_CODES` lookup `Club`'s country resolution already used, via `loaders.resolve_football_country_name`.
- [ ] **`etl/cli.py` still not built** — the single-player validation below used an ad-hoc script, not the real CLI. Needed before this can run over all 572 players instead of one.
- [x] **6. Run end-to-end against one real player** (Saka/Arsenal) as a proof of the full chain before scaling: parse → normalize → load → verified by re-reading from Postgres. All rows landed correctly: `Player`, `PlayerSourceMapping` (`match_method="seed"`), `PlayerStats` (goals/assists/minutes/appearances promoted, 52 keys in the `stats` JSONB catch-all), `PlayerMarketValue` (67 time-series points, 2018–2026), `PlayerAttributeProfile` (6 attributes).
- [ ] **Full-scale run against all 20 clubs / 572 players** — blocked on the `etl/cli.py` build and the stage-2 re-crawl above.
- [ ] **Fixture-based unit tests** (`tests/unit/fixtures/fotmob_*.{html,json}` + parser/normalizer tests) — still open, per the original plan's testing section.
